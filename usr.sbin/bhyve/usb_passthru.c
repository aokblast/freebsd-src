...
usb_passthru_probe(struct usb_hci *hci, const nvlist_t *nvl)
{
	...
	if (!libusb_is_init) {
		error = libusb_init_context(NULL, NULL, 0);
		if (error) {
...
usb_passthru_init(void *scarg)
{
	...
	if (pthread_mutex_init(&sc->mtx, &attr) != 0) {
    ...
    int epctx)
{
	...
	struct usb_passthru_ep_types *ep_types = &sc->endpoint_types[epctx << 1 | dir];

	ep_type = ep_types->type;
	dir = ep_types->inout;
	assert(ep_type != -1);
