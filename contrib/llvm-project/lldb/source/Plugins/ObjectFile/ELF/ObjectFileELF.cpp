...
ObjectFile::Strata ObjectFileELF::CalculateStrata() {
  ...
  case llvm::ELF::ET_EXEC:
	...
	SectionSP loader_section = section_list->FindSectionByName(loader_section_name);
	if (loader_section) {
	  char buffer[256];
	  size_t read_size = ReadSectionData(loader_section.get(), 0, buffer, sizeof(buffer));

	  // We compare the content of .interp section
	  // It will contains \0 when counting read_size, so the size needs to decrease by one
