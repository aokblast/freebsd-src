...
bool
decode_cfi(struct trapframe *tf, unsigned long *addr, uint32_t *type) {
	unsigned char buffer[13];

	strlcpy(tf->tf_addr - 12, buffer, sizeof(buffer));
	 ...
	 * if it is, we can identified it is the CFI fault
	 */
	...
	*addr = ((register_t *)tf)[regoff(((buffer[0] >> 2) & 0x1) << 3) | ((buffer[2] >> 3) & 7)];
	return true;
}
